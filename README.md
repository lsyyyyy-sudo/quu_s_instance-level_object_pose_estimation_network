...
| # | 任务 | 状态 |
|:---:|---|:---:|
| 1 | 获取目标物体 3D 模型并转换为 BOP 格式 | ✅ 完成（4 视图生成，三轴 99%/102%/101% 对官方尺寸）→ [MODEL_REGEN.md](docs/MODEL_REGEN.md) |
| 2 | 搭建 BlenderProc 渲染环境，跑通合成数据生成 | ✅ 完成（500 帧 / 5000 实例）→ [RENDER_SETUP.md](docs/RENDER_SETUP.md) · [DATASET_v1.md](docs/DATASET_v1.md) |
| 3 | 从 GT 位姿生成 8 角点 2D 标签 | ✅ 完成并验证（重投影一致、热图峰 **0 格**误差）→ [TRAINING.md](docs/TRAINING.md) §2 |
| 4 | 实现单图角点热图网络（含 BoxDreamer 的热图/损失配方） | ✅ 完成（ResNet18 + FPN，12.0 M 参数） |
| 5 | 在渲染数据上训练（含域随机化与数据增强） | 🟡 CPU 冒烟 + 小样本过拟合通过（`tr_loss` 56.5→8.8）；**待 GPU 正式训练** |
| 6 | 在目标视频上测试并可视化 | ⬜ 未开始（`head_left_rgb_raw.mp4`，可跑，但模型未正式训练） |
| 7 | 定量评估与误差分析 | 🟡 指标就绪（`corner_err_px` / `pck@t`），待训练模型 |
| 8 | ~~标定头戴相机内参 K~~ | ✅ **本项目不需要**（psd 确认 + 代码核实：网络与损失都不消费 K）→ [TRAINING.md](docs/TRAINING.md) §5.1 |

> 「完成」= 已在**真实数据**上跑通并有独立验证工具佐证（`scripts/verify_{bop,dataloader,pnp}.py`）。
> 当前自检规模：**56 个用例**，覆盖几何 / 角点热图 / 网络前向 / 损失 / 配置装配 / 数据集 IO / 仓库卫生。

### 已知缺陷（会影响最终效果，见 [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)）

| 编号 | 问题 |
|---|---|
| `DATA-16` | 训练集**遮挡严重不足**（95% 实例完全可见），HCCEPose 的遮挡鲁棒能力学不到 |
