...
| `raw/official/` | DJI 官方产品图 | 原始文件名即可 | 手工下载 |
...
| **2** | GIGAZINE 六面开箱图 | **角度最全**，补足视频里看不到的背面/侧面/底面 |
| **3** | DJI 官方产品图 | 几何标准，但**是渲染图**，光照过于理想 |
| 4 | 现成 3D 模型 | 只作几何对照，**不用作输入**（纹理缺失，且绕过考核第①步）|

**为什么优先视频 + 实拍**：3D 生成模型会从输入图里"学"纹理。用渲染图当输入，
...
| 2 | 视频里目标物体的清晰帧 | `data/raw/video_frames/` | ⬜ |
| 3 | GIGAZINE 六面图 | `data/raw/web_views/` | ⬜ |
| 4 | DJI 官方产品图 | `data/raw/official/` | ⬜ |
| 5 | 相机标定（棋盘格 + 内参） | `data/raw/calibration/` | ⛔ **需向 psd 索要** |
| 6 | 3D 生成模型输出 | `data/mesh/generated/` | ⬜ 依赖 2/3 |
