...
### 5.5 物体坐标系（BOP 8 个角点的基准）

**这是全流程里最容易被忽略、但影响最大的一个约定。** 8 个角点就是物体包围盒在该坐标系下的
8 个顶点，坐标系不定义清楚，角点顺序就没有物理意义，网络学到的对应关系也就无从谈起。

本项目的约定（已实测核对，2026-09-14）：

| 轴 | 含义 | 尺寸（官方 → 本模型实测） |
|---|---|---|
| **+X** | 长轴（机身宽度方向） | 70.5 → **69.94 mm** |
| **+Y** | 上（**顶部有录制键**那一侧） | 44.2 → **44.87 mm** |
| **+Z** | **正面（镜头方向）** | 32.8 → **33.09 mm** |

原点在包围盒中心。三轴误差都在 2% 以内（模型按官方**对角线** 89.44 mm 等比缩放）。

**怎么核对的**（`data/render_ws/axis_views.py`，纯 numpy+PIL、秒出，不需要 Cycles）：
把网格顶点按图集 UV 取色后，沿 ±X / ±Y / ±Z 六个方向做正交投影，直接看哪一面是什么：

- `cam at +Z (looking -Z)` → **正面**：前置屏在左、镜头在右、`ACTION 4` 字样在右下 —— 与真机一致
- `cam at +Y (looking -Y)` → **顶部**：录制键在一端，镜头鼓包在另一端
- 由此定出 `+Z` = 镜头方向、`+Y` = 上

⚠️ 一个容易误判的点：**惯性主轴和坐标轴差 6~8°**，看起来像"模型是歪的"。
这是**镜头偏心**造成的——镜头在一侧，重心偏移，惯性主轴自然跟着偏。
判据应该看**包围盒**是否紧贴物体，而不是看主轴。
实测包围盒 `69.94 / 44.87 / 33.09` 对官方 `70.5 / 44.2 / 32.8`，是非常紧的贴合。

⚠️ 另一个容易误判的点：**渲染图里模型是"躺着"的（镜头朝上）**。
这是物理模拟让它掉到地上之后自然停的姿态，**不是坐标系的问题**。
BOP PBR 数据本来就要求物体以**随机姿态**出现在场景里
（`sample_pose_func` 里就是 `bproc.sampler.uniformSO3()`）。

角点顺序由 `src/models/utils/box_utils.py` 的 `BB8_BITS` 定义（与 BoxDreamer 一致），
角点坐标由 `bbox3d_from_models_info()` 从 `models_info.json` 的 `min_*`/`max_*` 生成。
