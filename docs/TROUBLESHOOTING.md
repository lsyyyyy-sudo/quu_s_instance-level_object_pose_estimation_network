...
- **解法**：**本次未修**（用户决定先拿这版去跑阶段③④）。三条可选路线：
  1. 把 `sample_pose_func` 的 `x,y` 范围收紧到 `±0.05`，逼它们叠起来（改一行）
  2. 加一圈**低矮围栏**（4 面 ~5 cm 挡板）当容器 —— 这才是真正的 bin-picking
  3. 照 BoxDreamer 的路子：渲染时干净单体，**训练时在线合成**背景与遮挡
     （`refs/BoxDreamer/src/datasets/base.py` 的 `bg_images` / `occlusion_objs`）
- **影响文件**：`data/render_ws/gen_pbr_data_demo.py`（`sample_pose_func`）
- **教训**：
