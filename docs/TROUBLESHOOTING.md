...
- **时间**：2026-09-16 / 09-17（连踩两次）
- **现象**：脚本里写 `pgrep -f 'loss_ablation.sh'` 然后 `kill -9` 这些 pid，
  结果**脚本自己的 shell 一起死了**，`[remote] exit=-1`，后面的步骤全部没执行。
  另一次是 `pgrep -f 'exp_name=train_v4'` 报出"还有 2 个进程"，其实是它自己的命令行里
  含有这个字符串。
- **诊断**：`pgrep -f` 匹配的是**完整命令行**，而 `/bin/bash -c "... pgrep -f 'xxx' ..."`
  的**命令行本身就包含 `xxx`**。所以它会匹配到自己（有时还有父 shell）。
  同理 `pgrep -f 'run.py --config-name=train.yaml'` 会匹配到包含该串的编辑命令。
- **解法**：
  - 排除自己和父进程：`ME=$$; for p in $(pgrep -f PAT); do [ "$p" = "$ME" ] && continue; ... done`
  - 或者**用 pidfile**（最稳）：启动时 `echo $! > x.pid`，之后只认 pidfile
  - 或者用 `pgrep -x <进程名>`（精确匹配进程名，不匹配命令行）
  - 判断"还剩几个"时，**不要用 `pgrep -cf`，改用 `/proc/<pid>/cmdline` 逐个核对**
- **影响文件**：`%TEMP%\dsh_x1.sh`、`dsh_iso.sh`、多个驱动脚本
- **教训**：
  - ⚠️ **"还有 N 个进程在跑"这类判断，来源必须唯一且明确**（pidfile 或精确进程名），
    不能用模糊的字符串匹配 —— 它会同时骗过你两次：让你以为任务还在跑，
    又让你把自己的 shell 杀掉。
  - ⚠️ **诊断类脚本要用只读方式写。** 别在排查脚本里做 `kill` ——
    排查脚本自己出错时，损失会从"没查到"变成"把环境搞坏"。
